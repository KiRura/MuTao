...
function today(date) {
  ...
  const wareki = dt.toLocaleDateString("ja-JP-u-ca-japanese", {
    year: "numeric"
  });

  return `${y}年(${wareki})${("00" + (m + 1)).slice(-2)}月${("00" + d).slice(
    -2
  )}日(${dayOfWeek}) ${hour}時${min}分${sec}秒${msec}`;
}
/**
 *
 * @param {number} sec
...
function returnMusic(interaction) {
  ...
  if (!queue && interaction.guild.members.me.voice.channel)
    return "多分再起動したのでplayをするかvcから蹴るかして下さいな。";
...
try {
  client.once(Events.ClientReady, async client => {
    setInterval(async () => {
      ...
      client.user.setActivity({
        name: `${discordplayer.queues.cache.size} / ${
          (await client.guilds.fetch()).size
        } servers・${client.users.cache.size} users・${result.time}ms`,
        ...
        options: [
          {
            ...
            name: "mode",
            description:
              "track: 1曲だけ, queue: キューをループ, autoplay: おすすめの曲を勝手に追加し続ける(無限ループに陥りやすい), off: 解除",
            required: true,
            choices: [
              {
                name: "track",
        ...
        options: [
          {
            ...
            name: "emojiid",
            description:
              "絵文字のID(絵文字をそのまま入力すると分解した結果が返って来ます。)",
            required: true
          },
          {
            type: ApplicationCommandOptionType.String,
  ...
  client.on(Events.InteractionCreate, async interaction => {
    try {
      ...
      (
        ...
        ).channels.fetch("1163807475196252200")
      ).send({
        embeds: [
          new EmbedBuilder()
      ...
      if (command === "help") {
        ...
        const embed = new EmbedBuilder()
          ...
          .setFields([
            ...
            },
            {
              name: "WebSocket",
              value: `${
                client.ws.ping === -1 ? "none" : `${client.ws.ping} ms`
              }`,
              inline: true
            },
            {
        ...
        //   url.match("si=")
        // )
        //   url = url.substring(0, url.indexOf("si=")).slice(0, -1);
        ...
        // const queuesize = urlboolean
        //   ? (getqueue ? getqueue.getSize() : 0) + track.tracks.length
        //   : (getqueue ? getqueue.getSize() : 0) + 1;
        // const queuenumber = getqueue
        //   ? `${getqueue.getSize() + 1}番目に追加｜キュー内合計: ${queuesize}曲`
        //   : "再生開始";
        ...
        if (!queue && interaction.guild.members.me.voice.channel)
          return await interaction.reply({
            content: "多分再起動したのでplayをするかvcから蹴るかして下さいな。",
            ephemeral: true
          });
        if (!queue)
          return await interaction.reply({
        ...
        await wait(3); // そういう演出
        interaction.deleteReply().catch(_error => {});
        ...
        queue.node.pause()
          ? await interaction.reply("一時停止したよ")
          : await interaction.reply({
              content: "既に一時停止中だよ！",
              ephemeral: true
            }); // deferReply/followUpをするとephemeralが使えないらしい
        ...
        queue.node.resume()
          ? await interaction.reply("一時停止を解除したよ")
          : await interaction.reply({
              content: "一時停止がされてなかったよ",
              ephemeral: true
            });
        ...
        if (queue.getSize() === 0)
          return await interaction.reply({
            content: "キューの中は既に再生中の曲だけだよ！",
            ephemeral: true
          });
        const number = option.getInteger("number");
        let losttracks = queue.getSize();
        if (number !== null) {
          if (losttracks < number)
            return await interaction.reply({
              content: ataokanumber,
              ephemeral: true
            });
          await interaction.deferReply();
        ...
        await interaction.followUp(
          `${losttracks}曲がダイソンの手によってまっさらになったよ`
        );
        ...
        const tracks = queue.tracks
          ...
          .map((m, i) => {
            return `**${i + pageStart + 1}.** (${
              m.duration === "0:00" ? "ライブ" : m.duration
            }) [${
              m.title.length <= 20 ? m.title : `${m.title.substring(0, 20)}...`
            }](${m.url})`;
          });

        const length =
          (queue.estimatedDuration +
            (queue.currentTrack.durationMS - queue.node.streamTime)) /
          1000;
        const queuelength =
          ...
          embeds: [
            {
              ...
              description: `**再生中:** (${streamtime}) [${
                queue.currentTrack.title.length <= 20
                  ? queue.currentTrack.title
                  : `${queue.currentTrack.title.substring(0, 20)}...`
              }](${queue.currentTrack.url})\n\n${tracks.join("\n")}${
                queue.getSize() > pageEnd
                  ? `\n**...**\n**他:** ${queue.getSize() - pageEnd}曲`
                  : ""
              }`, // 表示したキューの後にいくつかの曲があったらその曲数を表示
              thumbnail: {
        ...
        if (number && (number < 1 || number > queue.getSize()))
          return await interaction.reply({
            content: ataokanumber,
            ephemeral: true
          });
        ...
        const embed = {
          embeds: [
            {
              description: `**再生${hold ? "予定" : "開始"}:**${
                t.title.length < 15
                  ? ` [${t.title}](${t.url})`
                  : `\n[${t.title}](${t.url})`
              }\n**リクエスト:** ${t.requestedBy.displayName} (${
        ...
        if (queue.getSize() !== 0)
          embed.embeds[0].title = `**残り:** ${
            queue.estimatedDuration === 0
              ? "ライブのみ"
        ...
        if (command === "nowp" || !num) {
          ...
          time =
            queue.node.getTimestamp().progress === Infinity
              ? "\n**ライブ配信**"
              : `\n**残り:** ${times(
          ...
          embeds: [
            {
              ...
              color: mutaoColor,
              footer: {
                text: `今までに${queue.history.getSize()}曲再生しました｜ボリューム: ${vol}%`
              }
            }
          ]
        });
        ...
        if (queue.repeatMode === mode.value)
          return await interaction.reply({
            content: "既にそのモードです！",
            ephemeral: true
          });
        ...
        let page = option.getInteger("page");
        if (page === null) page = 1;
        if (page > Math.floor(queue.history.getSize() / 10) + 1)
          return await interaction.reply({
            content: ataokanumber,
            ephemeral: true
          });
        if (queue.history.getSize() === 0)
          return await interaction.reply({
            content: "履歴はまだ保存されていません",
            ephemeral: true
          });
        ...
        if (queue.history.getSize() === 1) {
          ...
          const length = queue.history.tracks
            .toArray()
            .slice(0, queue.history.getSize() - 1)
            .map(m => {
              return m.durationMS;
            });
        ...
        trackslength =
          trackslength === 0 ? "ライブ" : times(trackslength / 1000);
          ...
          embeds: [
            {
              ...
              description: `**再生中:** (${
                ...
                queue.history.getSize() > pageStart * -1
                  ? `\n**...**\n**他:** ${
                      queue.history.getSize() + pageStart
                    }曲`
                  : ""
              }`,
              color: mutaoColor,
              ...
              footer: {
                text: `ページ: ${page}/${
                  Math.floor(queue.history.getSize() / 10) + 1
                }`
              }
            }
          ]
        ...
        await interaction.followUp(
          `${queue.tracks.data.length}曲をぐしゃぐしゃにしたよ！`
        );
        ...
        (await queue.node.seek(seek * 1000))
          ? await interaction.followUp(`${seek}秒に移動したよ！`)
          : await interaction.followUp(ataokanumber);
        ...
        if (group === "user") {
          if (manage === "add" || manage === "remove") {
            ...
            const content =
              manage === "add"
                ? `${user.displayName} への ${role.name} の付与が完了しました。`
                : `${user.displayName} から ${role.name} の剥奪が完了しました。`;
            await interaction.reply(content);
          } else if (manage === "list") {
            const size = user.roles.cache.size;
            if (size === 1)
              return await interaction.reply({
                content: `対象のユーザー ${user.displayName} にロールが付いていません。`,
                ephemeral: true
              });
              ...
              embeds: [
                {
                  ...
                  description: `${user.roles.cache
                    .map(role => {
                      if (role.name === "@everyone") return;
                      return `${role}`;
                    })
                    .join("\n")}\n**最上位:** ${highest} (${
                    rolesize - highest.rawPosition
                  } / ${rolesize})${
                    color !== null
          ...
          let content =
            manage === "add"
              ? `${membersize}人に ${role.name} の付与を開始します。`
              : `${membersize}人から ${role.name} の剥奪を開始します。`;
          await interaction.reply(content);
          ...
          content =
            manage === "add"
              ? `${membersize}人への ${role.name} の付与が完了しました。`
              : `${membersize}人からの ${role.name} の剥奪が完了しました。`;
          content =
            content + failed ? `\n権限的に無理だった人数: ${failed}` : "";
          ...
          interaction
            ...
            .catch(_error => {
              interaction.channel.send(content).catch(_error => {});
            });
        }
        ...
        try {
          ...
          if (
            ...
            embed.embeds[0].image = { url: image };
          const attachmentthumbnail = option.getAttachment(
            "attachmentthumbnail"
          );
          const urlthumbnail = option.getString("urlthumbnail");
          const thumbnail = attachmentthumbnail
          ...
          const authorimage = attachmentauthorimage
            ? attachmentauthorimage.url
            : urlauthorimage;
          if (
            authorimage !== null &&
            ...
            embed.embeds[0].author.icon_url = authorimage;
          const footertext = option.getString("footertext");
          const attachmentfooterimage = option.getAttachment(
            "attachmentfooterimage"
          );
          const urlfooterimage = option.getString("urlfooterimage");
          if (
            footertext !== null ||
            attachmentfooterimage !== null ||
            urlfooterimage !== null
          )
            embed.embeds[0].footer = {};
          if (footertext !== null) embed.embeds[0].footer.text = footertext;
          const footerimage = attachmentfooterimage
            ? attachmentfooterimage.url
            : urlfooterimage;
          if (
            footerimage !== null &&
            (footerimage.startsWith("http://") ||
              footerimage.startsWith("https://"))
          )
            embed.embeds[0].footer.icon_url = footerimage;
          const rgbcolor = option.getInteger("color");
          const hexcolor = option.getString("hexcolor");
          if (rgbcolor !== null || hexcolor !== null) {
            if (hexcolor === null || (hexcolor !== null && rgbcolor !== null))
              embed.embeds[0].color = rgbcolor;
            if (rgbcolor === null) {
              const color = parseInt(hexcolor, 16);
              if (!color)
                return await interaction.reply({
                  content: `カラーコード${hexcolor}は16進数である必要があります。`,
                  ephemeral: true
                });
              if (color > 16777215)
                return await interaction.reply({
                  content: `変換後のカラーコード(${color})が16777215を超えているため適応できません。`,
                  ephemeral: true
                });
          ...
          return await interaction.reply({
            content: `権限的か開発者のミスかそういう仕様で送信できませんでした。\n${error}`,
            ephemeral: true
          });
        }
          ...
          embeds: [
            {
              ...
              description: `${translated.text.substring(0, 4096)}${
                translated.text.length > 4096 ? "..." : ""
              }`,
              color: mutaoColor
            }
          ]
        ...
        const m = dt.getMonth();
        const d = dt.getDate();
        const mprog = Math.floor(
          (dt.getDate() / new Date(y, m + 1, 0).getDate()) * 100
        );
        const drem = new Date(y, m + 1, 0).getDate() - dt.getDate();
        const dprog = Math.floor(
          ((dt.getTime() - new Date(y, m, d).getTime()) /
            (24 * 60 * 60 * 1000)) *
            100
        );
        const mrem = Math.floor(
          (new Date(y, m, d + 1).getTime() - dt.getTime()) / 1000 / 60
        );
        const yprog = Math.floor(
          ((dt.getTime() - Date.parse(`${y - 1}/12/31`)) /
            (365 * 24 * 60 * 60 * 1000)) *
            100
        );
        const dyrem = Math.floor(
          (Date.parse(`${y}/12/31`) - dt.getTime()) / 1000 / 60 / 60 / 24
        );
        await interaction.reply(
          `${today(
            dt
        ...
        if (!image && !url)
          return await interaction.reply({
            content: "画像かURLを指定して下さい。",
            ephemeral: true
          });
        if (image && url)
          return await interaction.reply({
            content: "どちらか一方のみを指定して下さい",
            ephemeral: true
          });
        ...
        if (!interaction.guild)
          return await interaction.reply("サーバー内で実行して下さい");
        if (!interaction.member.voice.channel)
          return await interaction.reply({
            content: "VCに入ってから実行して下さい",
            ephemeral: true
          });
        ...
        if (!Number(emoji))
          return await interaction.reply(
            String(emoji).replace("<", "").replace(">", "").replace(":", "")
          );
        ...
        let vc = option.getChannel("vc");
        if (vc === null && interaction.member.voice.channel)
          return await interaction.reply({
            content: "VCを指定するかVCに入室して下さい。",
            ephemeral: true
          });
        if (vc === null) vc = interaction.member.voice.channel;
        if (!vc.members)
          return await interaction.reply({
            content: "指定先のVCには誰もいません。",
            ephemeral: true
          });
        ...
        const result = await (
          await fetch(`https://discord.com/api/users/${id}`, {
            method: "GET",
            headers: {
        ...
        if (!result.banner && !result.banner_color)
          return await interaction.reply({
            content: "バナー画像も色も取得できませんでした。",
            ephemeral: true
          });
        const url = result.banner
          ? `https://cdn.discordapp.com/banners/${id}/${result.banner}.${type}?size=4096`
          : null;
        const description = result.banner
          ? `[バナーの画像URL](${url})`
          : `バナーの色コード: ${result.banner_color}`;
        ...
        if (!role.iconURL())
          return await interaction.reply({
            content: "指定されたロールにアイコンはありませんでした。",
            ephemeral: true
          });
        ...
        await interaction.followUp(
          cancel
            ? `${membersize}人をスピーカーミュートしました。`
            : `${membersize}人のスピーカーミュートを解除しました。`
        );
        ...
        if (
          ...
          !me.permissionsIn(channel.id).has(PermissionFlagsBits.ManageMessages)
        )
          return await interaction.reply({
            content: "メッセージを管理する権限がありません！",
            ephemeral: true
          });
        ...
        await interaction.reply(
          `<#${channel.id}>内の${num}個のメッセージを削除しています...。`
        );
        const fetchreply = await interaction.fetchReply();

        try {
          await Promise.all(
            (await channel.messages.fetch({ limit: num })).map(
              async message => {
          ...
          return interaction.editReply("権限が変更されました。").catch(e => {
            return interaction.channel
              .send("権限が変更されたか、チャンネルが削除されました。")
              .catch(e => {});
          });
        }

        const finish = `<#${channel.id}>内の${num}個のメッセージを削除しました。`;
        interaction.editReply(finish).catch(e =>
          interaction.channel.send(finish).catch(error => {
            logger.error(error);
          })
        );
        interaction.user.send(finish).catch(error => {
          logger.error(error);
        });
      } else if (command === "deeplusage") {
        const result = await (
          await fetch("https://api-free.deepl.com/v2/usage", {
            method: "GET",
            headers: {
        ...
        await interaction.reply({
          embeds: [
            {
              description: `**今月の翻訳文字数:** ${
                result.character_count
              }文字\n**残り:** ${
                result.character_limit - result.character_count
              }文字`,
              color: mutaoColor
            }
          ]
        ...
        const ownercolor = owner.roles.color
          ? owner.roles.color.color
          : mutaoColor;

        if (geticon) {
          if (!iconurl)
            return await interaction.reply({
              content: "アイコンが設定されていません。",
              ephemeral: true
            });
        ...
        const joinNowTime = Math.floor(
          (new Date().getTime() - guild.joinedTimestamp) / 1000 / 60 / 60 / 24
        );
        ...
        await interaction.reply({
          content: crypto.randomBytes(n).toString("base64").substring(0, n),
          ephemeral
        });
        ...
        const channel = (await interaction.guild.channels.fetch()).filter(
        ...
        if (channel.size === ignore) {
          ...
          return await interaction.followUp(
            "全部既に設定しようとしたビットレートと同じでした"
          );
        } else if (channel.size === ignore + eq) {
          return await interaction.followUp(
            "全部権限不足か既に同じビットレートだったかでスキップされました。"
          );
        }
        let content = `${success}個のVCを${
          bitrate / 1000
        } Kbpsに設定しました。`;
        ...
        if (interaction.user.id !== "606093171151208448")
          return await interaction.reply("管理者及び開発者のみ実行可能です。");
      ...
      if (interaction.user.id !== "606093171151208448") {
        await client.users.cache
          .get("606093171151208448")
          .send(
            `${
              interaction.guild
                ? `${interaction.guild.name}(${interaction.guild.id})の${interaction.user.tag}`
          ...
          .catch(
            async e =>
              await interaction.channel
                ...
                .catch(
                  async e =>
                    await interaction.user.send(errormsg).catch(e => {})
                )
          );
      } else {
  ...
  client.on(Events.MessageCreate, async message => {
    ...
    if (
      message.guild.id === "1074670271312711740" &&
      message.author.id === "606093171151208448"
    ) {
  ...
  client.on(Events.GuildCreate, async guild => {
    try {
      ...
      await (
        await (
          await client.guilds.fetch("1074670271312711740")
        ).channels.fetch("1144550066531598426")
      ).send({
        embeds: [
          {
  ...
  try {
    discordplayer.events.on("playerTrigger", async queue => {
      if (
        ...
        return;
      await queue.guild.members.me.setNickname(
        `${queue.currentTrack.title.substring(0, 29)}${
          queue.currentTrack.title.length > 29 ? "..." : ""
        }`
      );
    });
